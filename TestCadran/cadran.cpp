 ...
 */
Cadran::Cadran(const int _posX, const int _posY, const int _hauteur, const int _largeur,const int _couleurFond,const int _couleurEcriture):
    ligne(_posX+1),
    colonne(_posY+1),
    hauteur(_hauteur),
    largeur(_largeur),
    couleurFond(_couleurFond),
    couleurEcriture(_couleurEcriture)
{
...
void Cadran::Afficher(const int _valeur, const int _position,const int _ligne)
{
    ...
    PositionnerCurseur(ligne+_ligne-1,colonne + _position);


    cout << "\e[0;"<<couleurEcriture<<";"<<couleurFond<<"m";
 ...
 * @details permet d'effacer le cadran
 */
void Cadran::Effacer()
{
...
void Cadran::Afficher(const string _texte, const int _position,const int _ligne)
{
    ...
    PositionnerCurseur(ligne+_ligne,colonne + _position);
